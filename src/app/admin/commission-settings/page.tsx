...
import type { MatchCommission } from '@/lib/types';

export default function MatchCommissionSettingsPage() {
  useSuperAdminOnly();
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(configRef, (doc) => {
      if (doc.exists()) {
        const data = doc.data() as MatchCommission;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: 'Match commission updated successfully!', className: 'bg-green-100 text-green-800' });
    } catch (error: any) {
      toast({
    ...
    <>
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-4">
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight flex items-center gap-2">
                <Percent className="h-7 w-7 sm:h-8 sm:w-8 text-primary" />
                Match Commission Settings
            </h2>
      </div>
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          <CardTitle>Set Match Commission</CardTitle>
          <CardDescription>
            This percentage will be taken by the platform from the total entry fee of each match.
          </CardDescription>
        </CardHeader>
