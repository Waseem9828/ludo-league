...
const UpiCard = ({ upi, onSetActive, onDelete, onEdit, isSaving }: { upi: UpiData, onSetActive: (id: string) => void, onDelete: (id: string) => void, onEdit: (upi: UpiData) => void, isSaving: boolean }) => (
  <div className={`p-4 border rounded-lg ${ upi.isActive ? 'text-primary-foreground bg-gradient-to-r from-primary-start to-primary-end' : 'bg-card' }`}>
...
export default function UpiManagementPage() {
  ...
  const handleUpdateUpi = async () => {
    if (!db) return;
    if (!editingUpi || !editingUpi.upiId.trim() || editingUpi.paymentLimit <= 0) {
      toast({ variant: 'destructive', title: 'Validation Error', description: 'UPI ID and a valid limit are required.' });
      return;
    }
    setIsSaving(true);
      ...
      <Card>
        ...
        <CardContent>
          ...
          {!loading && upiIds.filter((u) => u.isActive).length !== 1 && (
            <div className="mt-4 p-3 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 flex items-center gap-2"><AlertTriangle className="h-5 w-5" /><p className="text-sm font-semibold">Warning: There is currently no single active UPI ID. Users may not be able to make deposits. Please activate one.</p></div>
          )}
        </CardContent>
      </Card>

      {editingUpi && <EditUpiDialog editingUpi={editingUpi} setEditingUpi={setEditingUpi} onUpdate={handleUpdateUpi} isSaving={isSaving} />}
    </>
  );
}
...
function EditUpiDialog({ editingUpi, setEditingUpi, onUpdate, isSaving }: { editingUpi: UpiData, setEditingUpi: (upi: UpiData | null) => void, onUpdate: () => void, isSaving: boolean }) {
     return (
