...
const PrizeDistributionDialog = ({ tournament, isProcessing, setProcessing }: { tournament: Tournament, isProcessing: boolean, setProcessing: (val: boolean) => void }) => {
    const [distribution, setDistribution] = useState<{ [key: string]: number }>({ '1': tournament.prizePool });
    const { toast } = useToast();

    const handleDistribute = async () => {
        const totalDistributed = Object.values(distribution).reduce((sum, amount) => sum + Number(amount), 0);
        if (totalDistributed > tournament.prizePool) {
            toast({ title: 'Error', description: 'Total distributed prize cannot exceed the prize pool.', variant: 'destructive' });
            return;
        }

        const functions = getFunctions();
        const distributeFn = httpsCallable(functions, 'distributeTournamentWinnings');
        setProcessing(true);
        try {
            const result = await distributeFn({ tournamentId: tournament.id, prizeDistribution: distribution });
            toast({ title: 'Success', description: 'Prize distribution has been initiated.', className: 'bg-green-100 text-green-800' });
        } catch (error: any) {
    ...
    const handleDistributionChange = (rank: string, amount: string) => {
        setDistribution(prev => ({ ...prev, [rank]: Number(amount) }));
    };
    ...
    return (
        <Dialog>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Prize Distribution for {tournament.name}</DialogTitle>
                    <DialogDescription>
                        Define how the prize pool of ₹{tournament.prizePool} will be distributed. This action is final.
                    </DialogDescription>
                </DialogHeader>
                <div className="py-4 space-y-4">
                    {Object.entries(distribution).map(([rank, amount]) => (
                        <div key={rank} className="flex items-center gap-4">
                            <Label className="w-16">Rank {rank}</Label>
                            <Input type="number" value={amount} onChange={e => handleDistributionChange(rank, e.target.value)} />
                        </div>
                    ))}
                    <Button variant="outline" size="sm" onClick={() => handleDistributionChange(String(Object.keys(distribution).length + 1), '0')}>Add Rank</Button>
                </div>
                <DialogFooter>
...
export default function TournamentDetailPage() {
    ...
    try {
        await runTransaction(firestore, async (transaction) => {
            ...
            if (action === 'join') {
                ...
                if(currentUserData.walletBalance < fee) throw new Error("Insufficient wallet balance.");
        ...
        toast({ title: `Successfully ${action === 'join' ? 'joined' : 'left'} the tournament!`, className: "bg-green-100 text-green-800" });
    } catch(error: any) {
  ...
  const handleUpdateStatus = async (newStatus: Tournament['status']) => {
      ...
      try {
          ...
          toast({ title: `Tournament status updated to ${newStatus}`, className: "bg-green-100 text-green-800" });
      } catch (error: any) {
          ...
          {isAdmin && (
              <Card>
                  ...
                  <CardContent className="grid grid-cols-2 gap-2">
                      ...
                      <PrizeDistributionDialog tournament={tournament} isProcessing={isProcessing} setProcessing={setIsProcessing} />
                  </CardContent>
              </Card>
          )}
